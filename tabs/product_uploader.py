...
class ProductUploaderUI(QWidget):
    ...
    def __init__(self):
        ...
        if os.path.isfile(self.save_file) == False:
            ...
            self.save_naver_id = f.readline().strip()
            self.save_naver_pw = f.readline().strip()
    ...
    def save_button_clicked(self):
        ...
        naver_id = self.naver_id.text()
        naver_pw = self.naver_pw.text()
    ...
    #     self.apiDto = APIDto()
    #     self.apiDto.excel_file = self.excel_file_name.text()
    #     self.apiDto.naver_id = self.naver_id.text()
    #     self.apiDto.naver_pw = self.naver_pw.text()
    #     self.apiDto.media_path = self.media_path_name.text()

    #     self.store_thread = StoreThread()
    #     self.store_thread.log_msg.connect(self.log_append)
    #     self.store_thread.store_finished.connect(self.finished)
    #     self.store_thread.setapiDto(self.apiDto)
    ...
    def initUI(self):
        ...
        self.naver_id_label = QLabel("API KEY")
        self.naver_id = QLineEdit(f"{self.save_naver_id}")
        self.naver_pw_label = QLabel("API SECRET")
        self.naver_pw = QLineEdit(f"{self.save_naver_pw}")

        login_inner_layout = QVBoxLayout()
        login_inner_layout.addWidget(self.naver_id_label)
        login_inner_layout.addWidget(self.naver_id)
        login_inner_layout.addWidget(self.naver_pw_label)
        login_inner_layout.addWidget(self.naver_pw)
